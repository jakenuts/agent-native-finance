...
| `BETTER_AUTH_URL`     | Production yes                  | Public app URL used by Better Auth, OAuth redirects, and self-dispatch      |
| `BETTER_AUTH_SECRET`  | Production yes                  | Stable Better Auth signing secret                                          |
| `OAUTH_STATE_SECRET`  | Production recommended          | Dedicated short-lived OAuth state signing secret                           |
| `A2A_SECRET`          | Production recommended          | HMAC secret for Agent Native A2A and durable/background dispatch            |
| `SECRETS_ENCRYPTION_KEY` | Production recommended       | Encryption key for stored app/user secrets                                 |
| `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` | Agent runtime yes | LLM provider key for the in-app agent                                      |
| `PLAID_CLIENT_ID` / `PLAID_SECRET` / `PLAID_ENV` | Plaid sync yes | Plaid API credentials and environment (`sandbox` or `production`); can be set through setup scoped secrets or deploy env |
| `PLAID_WEBHOOK_URL`   | Plaid production recommended    | Public webhook endpoint, normally `<app-url>/api/plaid-webhook`            |
| `RECURLY_API_KEY`     | Recurly API automation optional | Recurly API key for automated projected-income refresh; manual entries and CSV import do not require it |
| `RECURLY_SUBDOMAIN`   | Optional                        | Recurly app subdomain for console links; defaults to `app`                 |
| `FINANCE_OWNER_EMAIL` | Optional                       | Owner email stamped on local/default data; defaults to `owner@finance.local` |
| `FINANCE_DISABLE_SCHEDULER` | Optional                 | Set to `true` for tests, one-off action checks, and ephemeral dev sandboxes |
| `FINANCE_BLOCK_SIGNUP` | Optional                      | Set to `true` in production after the owner account exists for single-tenant installs |
