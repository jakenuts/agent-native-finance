...
Finance has Personal/Business profiles (see `server/lib/profile.ts` and the "Profiles" section of `AGENTS.md`). If the feature adds a new table or a new list/analytics action, this area is not optional.

- **New tables that hold user data belong to a profile.** Add `profile TEXT NOT NULL DEFAULT 'personal'` to any new `fp_`-prefixed table's migration, alongside the rest of its columns — unless the table is a genuine exception (a single-owner settings/key-value store like `fp_settings`, or framework/migration bookkeeping like `fp_migrations`). When in doubt, add the column; it's cheap and additive, and an unscoped table is a bug waiting to happen.
- **List/analytics actions must route through `resolveEffectiveProfile`.** Any action that queries a profile-scoped table (`transactions`, `accounts`, `categories`, `rules`, `recurring`, `savedViews`, `budgetLines`, `paymentPlans`) for a set of rows — not a single row by id — needs a `profile: 'personal' | 'business' | 'all'` optional param and must call `resolveEffectiveProfile(db, owner, profile)` to build its filter. Follow `actions/list-transactions.ts` as the canonical pattern for param naming and the `eq(table.profile, resolved)` / cross-profile `'all'` filter shape.
- **Single-row-by-id actions are the normal exception.** An action like `get-transaction { id }` or `update-recurring { id, ... }` operates on one already-known row and does not need profile scoping — the id itself is the scope. Mutations that intentionally reassign a row's profile (e.g. `set-account-profile`) are also expected exceptions, not bugs.
- **Run the `profile-audit` action before considering the feature done.** It introspects every `fp_` table for a `profile` column and scans every `actions/*.ts` file for profile-scoped imports without a `resolveEffectiveProfile` call, returning `{ tables, actions, violations }`. A clean run has an empty `violations` array (real compliance, or entries explicitly documented in `profile-audit.ts`'s `ACTION_EXCEPTIONS`/`TABLE_EXCEPTIONS` maps) — don't ship a feature that turns `profile-audit` non-clean without adding a truthful exception entry.
